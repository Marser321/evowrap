...
const introImage = "https://images.unsplash.com/photo-1621135802920-133df287f89c?q=80&w=2070&auto=format&fit=crop";
const ctaImage = "https://images.unsplash.com/photo-1614162692292-7ac56d7f7f1e?q=80&w=2070&auto=format&fit=crop";
    ...
    const services = [
        {
            ...
            icon: Droplets,
            stats: data['ceramic-coating'].technicalSpecs.slice(0, 3).map(s => `${s.value} ${s.label}`)
        },
        {
            ...data['ppf'],
            image: data['ppf'].heroImage,
            icon: Shield,
            stats: data['ppf'].technicalSpecs.slice(0, 3).map(s => `${s.value} ${s.label}`)
        },
        {
            ...data['wrapping'],
            image: data['wrapping'].heroImage,
            icon: Zap,
            stats: data['wrapping'].technicalSpecs.slice(0, 3).map(s => `${s.value} ${s.label}`)
        },
        {
            ...data['detailing'],
            image: data['detailing'].heroImage,
            icon: Sparkles,
            stats: data['detailing'].technicalSpecs.slice(0, 3).map(s => `${s.value} ${s.label}`)
        }
    ];
